...
const SERVICE_NAME = process.env.SERVICE_NAME || 'cropfresh-service-auth';
const SERVICE_VERSION = process.env.SERVICE_VERSION || '1.0.0';
const ENVIRONMENT = process.env.NODE_ENV || 'development';

// SigNoz OTLP endpoint (in-cluster or local)
...
const sdk = new NodeSDK({
    ...
    traceExporter: new OTLPTraceExporter({
        url: OTEL_ENDPOINT,
        headers: {}, // Add auth headers if needed in production
    }),
    instrumentations: [
        getNodeAutoInstrumentations({
            // Auto-instrument: HTTP, Express, gRPC, Prisma, Redis
